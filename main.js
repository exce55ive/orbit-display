...
ipcMain.handle('gmail-auth-start', async () => {
  const cfg = loadOrbitConfig();
  const clientId = cfg?.integrations?.gmail?.clientId;
  const clientSecret = cfg?.integrations?.gmail?.clientSecret;
  if (!clientId) return { error: 'No Gmail client ID configured' };

  const http = require('http');
  const { shell } = require('electron');
  const redirectUri = 'http://127.0.0.1:8891/callback';
  const scope = 'https://www.googleapis.com/auth/gmail.readonly';
  const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&response_type=code&scope=${encodeURIComponent(scope)}&redirect_uri=${encodeURIComponent(redirectUri)}&access_type=offline&prompt=consent`;

  return new Promise((resolve) => {
    const server = http.createServer(async (req, res) => {
      if (!req.url.startsWith('/callback')) return;
      const params = new URL(req.url, 'http://127.0.0.1:8891').searchParams;
      const code = params.get('code');
      const error = params.get('error');
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.end('<html><body style="font-family:monospace;background:#060a10;color:#00d9ff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><h2>Gmail connected ✓ — you can close this tab.</h2></body></html>');
      server.close();
      ...
      try {
        ...
        if (tokens.access_token) {
          const currentCfg = loadOrbitConfig();
          if (!currentCfg.integrations.gmail) currentCfg.integrations.gmail = {};
          currentCfg.integrations.gmail.accessToken = tokens.access_token;
          currentCfg.integrations.gmail.refreshToken = tokens.refresh_token;
          currentCfg.integrations.gmail.tokenExpiry = Date.now() + (tokens.expires_in * 1000);
          saveOrbitConfig(currentCfg);
          resolve({ ok: true });
        } else {
          resolve({ error: tokens.error_description || 'Token exchange failed' });
    ...
    server.listen(8891, '127.0.0.1', () => shell.openExternal(authUrl));
    server.on('error', (e) => resolve({ error: 'Port 8891 in use: ' + e.message }));
    setTimeout(() => { try { server.close(); } catch {} resolve({ error: 'Auth timeout' }); }, 300000);
  });
});

ipcMain.handle('gmail-refresh-token', async () => {
  const cfg = loadOrbitConfig();
  const { clientId, clientSecret, refreshToken } = cfg?.integrations?.gmail || {};
  if (!refreshToken) return { error: 'No refresh token' };
  const fetch = await getFetch();
  const params = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: refreshToken, client_id: clientId, client_secret: clientSecret });
  try {
    const r = await fetch('https://oauth2.googleapis.com/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    const tokens = await r.json();
    if (tokens.access_token) {
      cfg.integrations.gmail.accessToken = tokens.access_token;
      cfg.integrations.gmail.tokenExpiry = Date.now() + (tokens.expires_in * 1000);
      saveOrbitConfig(cfg);
      return { ok: true, accessToken: tokens.access_token };
    }
    return { error: tokens.error_description || 'Refresh failed' };
...
// ─── IPC: OUTLOOK OAUTH (PKCE) ─────────────────────────────────────────────
ipcMain.handle('outlook-auth-start', async () => {
  const cfg = loadOrbitConfig();
  const clientId = cfg?.integrations?.outlook?.clientId;
  if (!clientId) return { error: 'No Outlook client ID configured' };

  const http = require('http');
  const crypto = require('crypto');
  const { shell } = require('electron');
  const redirectUri = 'http://127.0.0.1:8892/callback';
  ...
  const authUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${clientId}&response_type=code&scope=${encodeURIComponent(scope)}&redirect_uri=${encodeURIComponent(redirectUri)}&code_challenge=${challenge}&code_challenge_method=S256`;

  return new Promise((resolve) => {
    const server = http.createServer(async (req, res) => {
      if (!req.url.startsWith('/callback')) return;
      const params = new URL(req.url, 'http://127.0.0.1:8892').searchParams;
      const code = params.get('code');
      const error = params.get('error');
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.end('<html><body style="font-family:monospace;background:#060a10;color:#00d9ff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><h2>Outlook connected ✓ — you can close this tab.</h2></body></html>');
      server.close();

      if (error) { resolve({ error: params.get('error_description') || error }); return; }
      if (!code) { resolve({ error: 'No code received' }); return; }
      ...
      try {
        ...
        if (tokens.access_token) {
          const currentCfg = loadOrbitConfig();
          if (!currentCfg.integrations.outlook) currentCfg.integrations.outlook = {};
          currentCfg.integrations.outlook.accessToken = tokens.access_token;
          currentCfg.integrations.outlook.refreshToken = tokens.refresh_token;
          currentCfg.integrations.outlook.tokenExpiry = Date.now() + (tokens.expires_in * 1000);
          saveOrbitConfig(currentCfg);
          resolve({ ok: true });
        } else {
          resolve({ error: tokens.error_description || 'Token exchange failed' });
    ...
    server.listen(8892, '127.0.0.1', () => shell.openExternal(authUrl));
    server.on('error', (e) => resolve({ error: 'Port 8892 in use: ' + e.message }));
    setTimeout(() => { try { server.close(); } catch {} resolve({ error: 'Auth timeout' }); }, 300000);
  });
});

ipcMain.handle('outlook-refresh-token', async () => {
  const cfg = loadOrbitConfig();
  const { clientId, refreshToken } = cfg?.integrations?.outlook || {};
  if (!refreshToken) return { error: 'No refresh token' };
  const fetch = await getFetch();
  const scope = 'https://graph.microsoft.com/Mail.ReadBasic offline_access';
  const params = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: refreshToken, client_id: clientId, scope });
  try {
    const r = await fetch('https://login.microsoftonline.com/common/oauth2/v2.0/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    const tokens = await r.json();
    if (tokens.access_token) {
      cfg.integrations.outlook.accessToken = tokens.access_token;
      if (tokens.refresh_token) cfg.integrations.outlook.refreshToken = tokens.refresh_token;
      cfg.integrations.outlook.tokenExpiry = Date.now() + (tokens.expires_in * 1000);
      saveOrbitConfig(cfg);
      return { ok: true, accessToken: tokens.access_token };
    }
    return { error: tokens.error_description || 'Refresh failed' };
